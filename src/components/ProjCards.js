...
function ProjCards({ title, description,link }) {
  ...
  return (
    <div className=" border-2 border-black text-xl flex flex-col justify-around rounded-sm w-full md:w-[20em] my-4 h-72 py-4 px-2 hover:scale-[1.01] hover:shadow-xl transition-all">
      <font className="text-slate-800 text-2xl font-semibold w-full pb-2 border-b">
        {title ? title : "Project title here"}
      </font>
      <font className="font-mono text-lg text-slate-600 mt-2 h-36 overflow-hidden">
        {description
          ? description
